...
com.incture.formatter.dateFunctions = {
	...
	checkCondtionRecCase: function(selectedTabPath, selectedRowPath, oMatSectionModel, oDateFormat, oRecordMode) {
		...
		//Duplicating the inital array
		var initArray = jQuery.extend(true, [], oArray);
		if (oRecordMode === "CREATE") {
			initArray.splice(selectedRowPath, 1);
		} else {
			var index = parseInt(selectedRowPath, 10);
			var initArrayDates = this.getStartEndDateObjects(initArray[index]);
			initArrayDates[0].fieldValueNew = initArrayDates[0].uiPrevValue;
			initArrayDates[0].uiFieldValue = initArrayDates[0].uiPrevValue;
			initArrayDates[1].fieldValueNew = initArrayDates[1].uiPrevValue;
			initArrayDates[1].uiFieldValue = initArrayDates[1].uiPrevValue;
			initArray = this.sortConditionRecords(initArray);
		}
	...
	compareMultipleCondRec: function(validityStart, validityEnd, oContionRecord, oArray, initArray) {
		...
		var unchangedConditionRec = initArray; //jQuery.extend(true, [], oArray); //Duplicate of existing condition records
		...
		if (bValStartDate && bValEndDate) {
			...
			oTempArry = this.sortConditionRecords(oTempArry);
			this.bValArraySplit = false;
			var oParamObject = this.setDialogBoxParams(["0"], "PARTIAL_OVERLAP", [oContionRecord[0]], [oArray[0]], oTempArry, true,
		...
		for (var i = 0; i < arrLength; i++) {
			...
			switch (oValue) {
				...
				case "01":
					...
					oLoopParamObject = this.setDialogBoxParams(["0"], "SPLIT_RECORD", oSplitRecords, unchangedConditionRec[i], oTempArry, true,
						unchangedConditionRec);
				...
				case "02":
					...
					oLoopParamObject = this.setDialogBoxParams(["0"], "PARTIAL_OVERLAP", oExtEndDateRec, unchangedConditionRec[i], oTempArry, true,
						unchangedConditionRec);
					this.bValArraySplit = true;
					break;
				...
				case "03":
					...
					oLoopParamObject = this.setDialogBoxParams(["0"], "PARTIAL_OVERLAP", oExtStartDateRec, unchangedConditionRec[i], oTempArry, true,
						unchangedConditionRec);
					this.bValArraySplit = true;
				...
				case "04":
					...
					oLoopParamObject = this.setDialogBoxParams(["0"], "OVERLAP", oMergeRec, unchangedConditionRec[i], oTempArry, true,
						unchangedConditionRec);
					this.bValArraySplit = true;
					break;
				...
				case "06":
					var oCondRec = this.case6DatesInDiffRec(validityStart, validityEnd, oStartDate, oEndDate, oContionRecord[0], oArray[i],
						oNextConditRec);
				...
				case "09":
					...
					if (this.case9Split) {
						if (oTempArry.length) {
							...
							oLoopParamObject = this.setDialogBoxParams(["0"], "PARTIAL_OVERLAP", oContionRecord, oAffectedRec, oTempArry, true,
								unchangedConditionRec);
							oTempArry = this.sortConditionRecords(oTempArry);
							this.bValArraySplit = false;
							return oLoopParamObject;
